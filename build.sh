...
#
# Exceptions:
# 1: If the argument is "svg", this script will only generate
#    the svg file and omit the rest.
# 2: An additional argument of "active" may also be specified in
#    order to generate a cladogram with only active distributions.
...
active_build(){ echo "Generating active data"
	./gldt_active.sh $PROJNAME'.csv' $PROJNAME'_active.csv'
...
echo "$VERS" | grep -Fxqi "svg" && { $GC $PROJNAME.csv svg $CONFNAME.conf
	exit 0;}

# Run gnuclad and abort on error.
CHECK=`$GC $PROJNAME.csv $PROJNAME$VERS.svg $CONFNAME.conf`
echo -e "$CHECK"
